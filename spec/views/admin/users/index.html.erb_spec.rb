...
describe "/admin/users/index.html.erb" do
  ...
  it "should render list of users" do
    ...
    response.should have_tag("h1",  :text => "Users")
    response.should have_tag("tr>td", "MyString", 2)
    response.should have_tag("tr>td", "MyString", 2)
    response.should have_tag("tr>td", "MyString", 2)
  end
  
  it "should show title for a list of blog users" do 
    @blog = mock_model(Blog, :title => 'test blog')
    @blog.stub!(:creator).and_return(@user_98)              
    assigns[:blog] = @blog
    render "/admin/users/index.html.erb"   
    response.should have_tag("h1",  :text => "test blog Users")
  end
end
